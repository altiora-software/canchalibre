...
const SportComplexCard = ({ complex, onViewDetails }: SportComplexCardProps) => {
  ...
  return <button type="button" onClick={() => onViewDetails(complex)} className="group grid w-full grid-cols-[132px_minmax(0,1fr)_auto] overflow-hidden rounded-xl border border-slate-200 bg-card text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md dark:border-slate-700 sm:grid-cols-[156px_minmax(0,1fr)_auto]">
    ...
    <div className="min-w-0 space-y-2 px-3 py-3 sm:px-4"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate font-bold text-foreground">{complex.name}</h3>{sports.map((sport) => <Badge key={sport} className="rounded-md bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-50 dark:bg-emerald-950 dark:text-emerald-300">{sport}</Badge>)}</div><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{complex.neighborhood || "Zona a confirmar"}</span><span className="truncate">{complex.address || "Dirección a confirmar"}</span></div>{labels.length > 0 && <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-emerald-800 dark:text-emerald-300">{labels.map((amenity) => <span key={amenity} className="flex items-center gap-1">{iconForAmenity(amenity)}{amenity}</span>)}</div>}</div>
