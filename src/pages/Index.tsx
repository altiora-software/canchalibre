...
import { ArrowRight, Building2, Loader2, LocateFixed, MapPin, MessageCircle, RotateCcw, Search } from "lucide-react";
...
const Index = () => {
  ...
  const updateCriteria = (patch: Partial<DiscoveryCriteria>) => {
    ...
    if (next.view !== "map") nextParams.set("view", next.view);
  ...
  const selectComplex = (complex: SportComplexData) => {
    ...
    window.setTimeout(() => document.getElementById(`complex-${complex.id}`)?.scrollIntoView({ behavior: "smooth", block: "nearest" }), 0);
  };
  ...
  const handleResultsScroll = () => {
    ...
    if (container && hasMore && !loadingMore && container.scrollTop + container.clientHeight >= container.scrollHeight - 160) void loadMore();
  };
  ...
  return <><Helmet><title>Cancha Libre | Encontrá dónde jugar en Jujuy</title><meta name="description" content="Encontrá complejos deportivos en San Salvador de Jujuy." /><meta property="og:image" content="/hero-canchalibre-night.png" /></Helmet><div className="min-h-screen bg-background"><Header /><main>
    ...
    <section id="results" className="container mx-auto scroll-mt-20 px-4 pb-12 pt-6"><div className="mb-3"><h2 className="text-2xl font-bold tracking-tight">Complejos para jugar</h2><p className="mt-1 text-sm text-muted-foreground">Encontrá opciones según el deporte y los filtros seleccionados.</p></div><div className="mb-3 flex flex-wrap items-center gap-2"><Button type="button" variant={userLocation ? "secondary" : "outline"} size="sm" onClick={useMyLocation} disabled={locating}><LocateFixed className="mr-2 h-4 w-4" />{locating ? "Buscando ubicación…" : userLocation ? "Ordenados por cercanía" : "Usar mi ubicación"}</Button>{userLocation && <Button type="button" variant="ghost" size="sm" onClick={() => { setUserLocation(null); setLocationError(null); }}>Quitar ubicación</Button>}{locationError && <p role="status" className="text-sm text-destructive">{locationError}</p>}</div><ResultsToolbar count={filtered.length} criteria={criteria} onChange={updateCriteria} />
      {loading ? <div className="flex min-h-64 items-center justify-center gap-2 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" />Cargando complejos…</div> : error ? <Card className="mt-6"><CardContent className="p-8 text-center"><p className="font-medium">No pudimos cargar los complejos.</p><p className="mt-2 text-sm text-muted-foreground">Probá de nuevo en unos minutos.</p></CardContent></Card> : filtered.length === 0 ? <Card className="mt-6 border-dashed"><CardContent className="p-10 text-center"><Search className="mx-auto h-7 w-7 text-emerald-700" /><h3 className="mt-3 text-lg font-semibold">Todavía no encontramos complejos así</h3><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Estamos sumando nuevos espacios. Probá ampliar la zona o quitar características.</p><Button className="mt-5" variant="outline" onClick={() => setParams({}, { replace: true })}><RotateCcw className="mr-2 h-4 w-4" />Limpiar búsqueda</Button></CardContent></Card> : <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,.98fr)_minmax(390px,.9fr)]"><div ref={resultsScrollRef} onScroll={handleResultsScroll} className="max-h-[680px] space-y-2 overflow-y-auto pr-2">{filtered.map((complex) => <SportComplexCard key={complex.id} complex={complex} selected={complex.id === selectedComplexId} distanceKm={userLocation ? distanceInKm(userLocation, complex) : null} onSelect={selectComplex} onViewDetails={(item) => navigate(`/complex/${item.id}`)} />)}{loadingMore && <div className="flex justify-center py-4 text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Cargando más complejos…</div>}{hasMore && !loadingMore && <Button variant="outline" className="w-full bg-card" onClick={() => void loadMore()}>Cargar más complejos <ArrowRight className="ml-2 h-4 w-4" /></Button>}</div><div className="lg:sticky lg:top-20 lg:h-[590px]"><MapSection complexes={filtered} selectedComplexId={selectedComplexId} userLocation={userLocation} onSelectComplex={selectComplex} onViewDetails={(complex) => navigate(`/complex/${complex.id}`)} /></div></div>}
    </section>
